...
{#await size}}
	<span
		class="fixed top-0 left-0 z-3 h-full w-full bg-main-black"
...
{:then s}
	...
	{:then m}
		{#if mouse.current.x > 0}
			<div class="absolute top-0 left-0 z-0 flex h-screen w-full flex-col justify-around">
				{#each new Array(Math.floor(s.height / 100)) as _, i (i)}
					<div class="flex justify-around">
						{#each new Array(Math.floor(s.width / 100)) as _, j (j)}
							<span
								class="rounded-xs bg-main-white px-1 text-xs text-main-black"
								transition:fade|global={{
									duration: 300,
									delay: Math.hypot(Math.abs(j - 5), Math.abs(i - 5)) * 300
								}}
								use:fieldPoint={m}
							>
								Alex's Blog
	...
	<div class="relative z-10 mx-auto box-content max-w-200 px-10">
		<div class="flex h-screen items-center justify-center">
			<div class="padding-sm rounded-md bg-main-black" use:mask>
