<script lang="ts">
	...
	let fieldPoint: Action<HTMLSpanElement> = (node) => {
		...
		$effect(() => {
			...
			node.style.transform = `rotate(${newdeg}deg) rotateY(${currentScale}deg) rotateX(${currentScale}deg)`;
		});
	};
</script>
...
{#if mouse.current.x > 0 && maskSize && winSizeSynced}
	<div
		class="fixed top-0 left-0 z-0 box-border flex h-screen w-full flex-col justify-between px-5 py-20 md:px-30 md:py-20"
	>
		{#each new Array(Math.floor(windowHeight / patternDistance)) as _, i (i)}
			<div class="flex justify-between">
				{#each new Array(Math.floor(windowWidth / patternDistance)) as _, j (j)}
					...
					>
						<svg
							class="fill-main-green"
							width="34"
							height="24"
							in:scale|global={{
								duration: 300,
								delay: Math.hypot(Math.abs(j), Math.abs(i)) * 150
							}}
						>
...
<div class="pt-[35vh]">
	<div class="relative z-5 mx-auto box-content max-w-200 px-5 pb-3" use:mask>
		<div class="px-2 py-5" onresize={updateMask}>
			<h1 class="mb-5">Alex's Blog</h1>
			<Squiggle>
				<p class="p-5">
					Hi I'm Alex, and this is my blog. Here you can find all kinds of ramblings ~ from
					experiments in tech, to life updates, to opinion pieces. Whatever you're looking for, I
